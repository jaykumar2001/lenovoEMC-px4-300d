/*
 *  lpc_ich_lenovo.c - LPC ICH MFD + ST7565R LCD for Lenovo EMC PX-300d
 *
 *  Combines Intel ICH LPC bridge MFD support with a framebuffer driver
 *  for the 128x64 monochrome LCD on the PX-300d front panel.
 *
 *  LCD GPIO mapping (ICH9 southbridge GPIO, directly wired):
 *    SI=GPIO6, SCL=GPIO7, RS=GPIO16, A0=GPIO19, CS1=GPIO21
 *
...
/* Legacy ioctl compatibility with ums8485md */
struct lcm_member {
	int ctrl;
	int page;
	int column;
...
#define LCM_CMD_PON		0x01
#define LCM_CMD_POFF		0x02
#define LCM_CMD_RESET		0x03
#define LCM_CMD_DISP_NORMAL	0x04
#define LCM_CMD_DISP_REVERSE	0x05
#define LCM_CMD_ENTIRE_DISP_ON	0x06
#define LCM_CMD_ENTIRE_DISP_OFF	0x07
#define LCM_CMD_WRITE_DATA	0x0c

#define IOCTL_DISPLAY_COMMAND	_IOWR(0xF4, 0, struct lcm_member)

/* LCD init sequence for ST7565R */
static const u8 lcd_initdata[] = {
	0xa0, 0x2f, 0xa2, 0xc8, 0x27, 0x81, 0x17, 0xaf
};

/* Module parameters */
static bool lcd_enable = true;
module_param(lcd_enable, bool, 0444);
MODULE_PARM_DESC(lcd_enable, "Enable LCD framebuffer (default: true)");

static int lcd_contrast = 0x17;
module_param(lcd_contrast, int, 0644);
MODULE_PARM_DESC(lcd_contrast, "LCD contrast 0-63 (default: 23)");

static int lcd_refresh_ms = 200;
...
static bool px300d_detected;

/* ================================================================
 * LPC ICH structures (unchanged from original)
 * ================================================================ */
...
/* ================================================================
 * LCD: ICH9 GPIO helpers
 * ================================================================ */

static u32 ich9_gpio_base;

static inline void lcd_pin_set(struct px300d_lcd *lcd, u32 pin_mask, int value)
{
	u32 lvl = inl(lcd->gpio_base + ICH9_GP_LVL);

	if (value)
		lvl |= pin_mask;
	else
		lvl &= ~pin_mask;
	outl(lvl, lcd->gpio_base + ICH9_GP_LVL);
}

/* ================================================================
 * LCD: SPI bit-bang over GPIO
 * ================================================================ */

static void lcd_spi_send(struct px300d_lcd *lcd, bool is_data,
			  const u8 *data, int len)
{
	int i, bit;

	lcd_pin_set(lcd, LCD_BIT_CS1, 0);
	lcd_pin_set(lcd, LCD_BIT_A0, is_data ? 1 : 0);

	for (i = 0; i < len; i++) {
		u8 byte = data[i];
		for (bit = 7; bit >= 0; bit--) {
			lcd_pin_set(lcd, LCD_BIT_SCL, 0);
			lcd_pin_set(lcd, LCD_BIT_SI, (byte >> bit) & 1);
...
static void lcd_write_cmd(struct px300d_lcd *lcd, u8 cmd)
{
	lcd_spi_send(lcd, false, &cmd, 1);
}

static void lcd_write_data(struct px300d_lcd *lcd, const u8 *data, int len)
{
	lcd_spi_send(lcd, true, data, len);
}

static void lcd_hw_init(struct px300d_lcd *lcd)
{
	u32 use_sel, io_sel;
	int i;
...
static void lcd_clear_display(struct px300d_lcd *lcd)
{
	...
	for (page = 0; page < LCD_PAGES; page++) {
		lcd_write_cmd(lcd, ST7565_SET_PAGE | page);
		lcd_write_cmd(lcd, ST7565_SET_COL_LO | 0);
		lcd_write_cmd(lcd, ST7565_SET_COL_HI | 0);
		lcd_write_data(lcd, zeros, LCD_WIDTH);
	}
}

/*
 * Convert Linux 1bpp linear framebuffer to ST7565 page format and send.
 * FB format: 1 bit/pixel, MSB-first in each byte, left-to-right rows.
 * ST7565: 8 pages of 128 columns, each column byte is a vertical 8-pixel
 *         strip with bit 0 = top pixel.
 */
static void lcd_update_display(struct px300d_lcd *lcd)
{
	u8 page_buf[LCD_WIDTH];
	u8 *fb;
	int page, col, bit;

	if (!lcd->fb_info || !lcd->fb_info->screen_buffer)
		return;

	fb = lcd->fb_info->screen_buffer;

	if (memcmp(lcd->shadow, fb, LCD_FB_SIZE) == 0)
		return;
	memcpy(lcd->shadow, fb, LCD_FB_SIZE);
	...
	for (page = 0; page < LCD_PAGES; page++) {
		for (col = 0; col < LCD_WIDTH; col++) {
			...
			for (bit = 0; bit < 8; bit++) {
				int row = page * 8 + bit;
				int byte_off = row * (LCD_WIDTH / 8) + col / 8;
				int bit_pos = 7 - (col % 8);
				if (fb[byte_off] & (1 << bit_pos))
					val |= (1 << bit);
			}
			page_buf[col] = val;
		}
		...
		lcd_write_cmd(lcd, ST7565_SET_COL_LO | 0);
		lcd_write_cmd(lcd, ST7565_SET_COL_HI | 0);
		lcd_write_data(lcd, page_buf, LCD_WIDTH);
	}
...
static const struct fb_ops lcd_fb_ops = {
	.owner		= THIS_MODULE,
	.fb_read	= fb_sys_read,
...
static struct fb_var_screeninfo lcd_fb_var = {
	.xres		= LCD_WIDTH,
	.yres		= LCD_HEIGHT,
...
static struct fb_fix_screeninfo lcd_fb_fix = {
	.id		= "px300d-lcd",
...
/* ================================================================
 * LCD: legacy /dev/lcm misc device (ioctl compat with ums8485md)
 * ================================================================ */

static long lcm_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	struct lcm_member info;
	struct px300d_lcd *lcd = px300d_lcd_dev;
	int i;

	if (!lcd || !lcd->active)
		return -ENODEV;

	if (cmd != IOCTL_DISPLAY_COMMAND)
		return -ENOTTY;

	if (copy_from_user(&info, (void __user *)arg, sizeof(info)))
		return -EFAULT;

	if (info.page < 0 || info.page > 7 ||
	    info.column < 0 || info.column > 127)
		return -EINVAL;

	mutex_lock(&lcd->lock);

	lcd_write_cmd(lcd, ST7565_SET_PAGE | info.page);
	lcd_write_cmd(lcd, ST7565_SET_COL_LO | (info.column & 0x0f));
	lcd_write_cmd(lcd, ST7565_SET_COL_HI | ((info.column >> 4) & 0x0f));

	if (info.ctrl != LCM_CMD_RESET) {
		for (i = 0; i < info.size && i < 128; i++)
			lcd_write_data(lcd, &info.data[i], 1);
	}

	switch (info.ctrl) {
	case LCM_CMD_PON:
		lcd_write_cmd(lcd, 0xaf);
		break;
	case LCM_CMD_POFF:
		lcd_write_cmd(lcd, 0xae);
		break;
	case LCM_CMD_RESET:
		for (i = 0; i < ARRAY_SIZE(lcd_initdata); i++)
			lcd_write_cmd(lcd, lcd_initdata[i]);
		...
		break;
	case LCM_CMD_DISP_NORMAL:
		lcd_write_cmd(lcd, 0xa6);
		break;
	case LCM_CMD_DISP_REVERSE:
		lcd_write_cmd(lcd, 0xa7);
		break;
	case LCM_CMD_ENTIRE_DISP_ON:
		lcd_write_cmd(lcd, 0xa5);
		break;
	case LCM_CMD_ENTIRE_DISP_OFF:
		lcd_write_cmd(lcd, 0xa4);
		break;
...
static struct miscdevice lcm_misc = {
	.minor	= 128,
	.name	= "lcm",
	.fops	= &lcm_fops,
};
...
/* ================================================================
 * LCD: refresh work + init/cleanup
 * ================================================================ */

static void lcd_refresh_work_fn(struct work_struct *work)
{
	struct px300d_lcd *lcd = container_of(to_delayed_work(work),
					      struct px300d_lcd, refresh_work);
	if (!lcd->active)
		return;
	lcd_update_display(lcd);
	schedule_delayed_work(&lcd->refresh_work,
			      msecs_to_jiffies(lcd_refresh_ms));
}

static int lcd_try_init(void)
{
	struct px300d_lcd *lcd;
	struct fb_info *info;
	void *fb_buf;
	int ret;

	if (!ich9_gpio_base) {
		pr_warn("lcd: ICH9 GPIOBASE not available\n");
		return -ENODEV;
	}
	...
	fb_buf = vzalloc(PAGE_ALIGN(LCD_FB_SIZE));
	if (!fb_buf) {
		ret = -ENOMEM;
		goto err_fb_alloc;
	}

	info->screen_buffer = fb_buf;
	info->screen_size = LCD_FB_SIZE;
	info->fbops = &lcd_fb_ops;
	info->var = lcd_fb_var;
	info->fix = lcd_fb_fix;
	info->fix.smem_len = PAGE_ALIGN(LCD_FB_SIZE);
	info->par = lcd;
	info->flags = FBINFO_VIRTFB;

	ret = register_framebuffer(info);
	if (ret) {
		pr_err("lcd: register_framebuffer failed: %d\n", ret);
		goto err_fb_buf;
	}

	lcd->fb_info = info;
	memset(lcd->shadow, 0xff, LCD_FB_SIZE);
	...
	INIT_DELAYED_WORK(&lcd->refresh_work, lcd_refresh_work_fn);
	schedule_delayed_work(&lcd->refresh_work,
			      msecs_to_jiffies(lcd_refresh_ms));

	pr_info("lcd: ST7565R 128x64 framebuffer on ICH9 GPIO (base=0x%04x) registered as fb%d\n",
		lcd->gpio_base, info->node);
	return 0;
...
static void lcd_cleanup(void)
{
	...
	if (lcd->fb_info) {
		...
		vfree(lcd->fb_info->screen_buffer);
		framebuffer_release(lcd->fb_info);
	}
...
/* ================================================================
 * LPC ICH functions (unchanged from original)
 * ================================================================ */
				...
				const struct pci_device_id *id)
{
	...
	/* Read ICH9 GPIOBASE for LCD SPI bit-bang */
	if (priv->chipset >= LPC_ICH9 && priv->chipset <= LPC_ICH9ME) {
		u32 base_cfg;

		pci_read_config_dword(dev, GPIOBASE_ICH6, &base_cfg);
		ich9_gpio_base = base_cfg & 0xFFC0;
		if (ich9_gpio_base)
			dev_info(&dev->dev, "ICH9 GPIOBASE = 0x%04x\n",
				 ich9_gpio_base);
	}
...
static int __init lpc_ich_lenovo_init(void)
{
	...
	ret = pci_register_driver(&lpc_ich_driver);
	if (ret)
		return ret;
...
module_init(lpc_ich_lenovo_init);
module_exit(lpc_ich_lenovo_exit);

MODULE_AUTHOR("Aaron Sierra <asierra@xes-inc.com>");
MODULE_DESCRIPTION("LPC ICH MFD + ST7565R LCD for Lenovo EMC PX-300d");
MODULE_LICENSE("GPL");
